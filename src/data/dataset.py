...
class ImageDataset:
    ...
    def __init__(self, train_dir, val_dir, test_dir=None, image_size=(224, 224), batch_size=32,
        ...
        self.train_dir, self.val_dir = Path(train_dir), Path(val_dir)
        self.test_dir = Path(test_dir) if test_dir else None
        self.image_size, self.batch_size = tuple(image_size), batch_size
        self.shuffle_buffer, self.cache_dataset, self.prefetch_buffer = shuffle_buffer, cache_dataset, prefetch_buffer
        self.class_names = self.discover_classes(); self.num_classes = len(self.class_names)
    ...
    def _make(self, root, shuffle=False, augmentation_pipeline=None, augment=False):
        ...
        ds = tf.keras.utils.image_dataset_from_directory(root, labels='inferred', label_mode='int', class_names=self.class_names,
            image_size=self.image_size, batch_size=self.batch_size, shuffle=shuffle, seed=42)
    ...
    def get_class_weights(self):
        counts=self.count_images('train'); total=sum(counts.values())
        return {i:total/(self.num_classes*max(counts[c],1)) for i,c in enumerate(self.class_names)} if total and self.num_classes else None
...
def create_dataset_from_config(config=None):
    ...
    cfg=config or get_config(); d=cfg.dataset
    return ImageDataset(d.get('train_dir','data/train'),d.get('val_dir','data/val'),d.get('test_dir','data/test'),tuple(d.get('image_size',[224,224])),d.get('batch_size',32),d.get('shuffle_buffer',1000),d.get('cache_dataset',False),d.get('prefetch_buffer',32))
...
def create_segmentation_dataset(image_dir, mask_dir, image_size=(256,256), batch_size=8, num_classes=None, shuffle=False):
    """Pair images and masks by filename; masks contain integer class IDs."""
    ...
    def load(ip,mp):
        x=tf.io.decode_image(tf.io.read_file(ip),channels=3,expand_animations=False); m=tf.io.decode_image(tf.io.read_file(mp),channels=1,expand_animations=False)
        x=tf.image.resize(tf.cast(x,tf.float32),image_size)/255.; m=tf.cast(tf.squeeze(tf.image.resize(tf.cast(m,tf.float32),image_size,method='nearest'),-1),tf.int32)
...
def create_ocr_dataset(annotation_file, image_root=None, image_size=(32,256), batch_size=16, charset=None, max_length=32, shuffle=False):
    """Load JSON/JSONL records containing image and text fields."""
    p=Path(annotation_file); rows=[json.loads(x) for x in p.read_text(encoding='utf-8').splitlines() if x.strip()] if p.suffix.lower()=='.jsonl' else _load_json(p)
    if isinstance(rows,dict): rows=rows.get('annotations',rows.get('items',[]))
    root=Path(image_root) if image_root else p.parent; charset=list(charset or sorted(set(''.join(str(r['text']) for r in rows)))); lookup={c:i+1 for i,c in enumerate(charset)}
    images=[]; labels=[]
    for r in rows:
        text=str(r['text'])
...
def create_detection_dataset(annotation_file, image_root=None, image_size=(320,320), batch_size=8, num_classes=1, prediction_slots=100, shuffle=False):
    """Load COCO detection annotations into fixed SSD target slots.

    This adapter provides normalized [ymin,xmin,ymax,xmax] targets and one-hot class targets.
    It intentionally does not pretend to implement anchor matching; production SSD training
    should use an encoder that matches the model's anchor geometry.
    """
    coco=_load_json(Path(annotation_file)); root=Path(image_root) if image_root else Path(annotation_file).parent
    images={i['id']:i for i in coco.get('images',[])}; grouped={i:[] for i in images}
    for a in coco.get('annotations',[]):
        if a.get('image_id') in grouped: grouped[a['image_id']].append(a)
    records=[]
    for image_id,info in images.items(): records.append((str(root/info['file_name']),grouped[image_id],float(info['width']),float(info['height'])))
    ...
    def load(ip,obj_json,w,h):
        ...
        def encode(objs,wv,hv):
            import numpy as np
            b=np.zeros((prediction_slots,4),np.float32); c=np.zeros((prediction_slots,num_classes),np.float32)
            for j,a in enumerate(objs[:prediction_slots]):
                bx,by,bw,bh=a['bbox']; b[j]=[by/hv,bx/wv,(by+bh)/hv,(bx+bw)/wv]
                cid=int(a.get('category_id',1))-1
                if 0<=cid<num_classes: c[j,cid]=1.
            return b,c
        b,c=tf.numpy_function(encode,[tf.io.decode_json_example(obj_json) if False else obj_json,w,h],[tf.float32,tf.float32])
        b.set_shape((prediction_slots,4)); c.set_shape((prediction_slots,num_classes))
        return x,{'boxes':b,'classes':c}
